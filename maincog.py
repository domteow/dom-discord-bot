...
class YTDLSource(discord.PCMVolumeTransformer):
    def __init__(self, source, *, data, volume=0.5, requester):
        ...
        # YTDL info dicts (data) have other useful information
...
class Music(commands.Cog):
    ...
    @commands.command(name='jbuy', description="fuck jbuy")
    async def jbuy_(self, ctx):
        # vc = ctx.voice_client

        # if not vc or not vc.is_connected():
        embed = discord.Embed(title="", description="fuck jbuy", color=discord.Color.green())
        return await ctx.send(embed=embed)
...
#     bot.run("OTc4NDg4MjY2NjAyOTg3NTcw.GEibU0.eznyMw66KwKx3gJwomAmJTIr1F6vuffITOgvJs")